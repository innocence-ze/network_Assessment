...
public static class NetManager 
{
    ...
    static void OnReceivingData()
    {
        ...
        if (receiveBuffer.Length < bodyLength)
            return;
        receiveBuffer.readIndex += 2;
        //decode name
        string protoName = MsgBase.DecodeName(receiveBuffer.bytes, receiveBuffer.readIndex, out int nameCount);
        if (protoName == "")
        {
        ...
        MsgBase msg = MsgBase.Decode(receiveBuffer.bytes, readIndex, bodyCount);
